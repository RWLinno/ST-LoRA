...
# 训练模型并记录指标
def train(model, g, optimizer, criterion, num_epochs):
    for epoch in range(num_epochs):
        ...
        accuracy = accuracy_score(g.ndata['label'].numpy(), predictions.numpy())
...
def load_adj(pkl_filename, adjtype):
    sensor_ids, sensor_id_to_ind, adj_mx = load_adj_from_pickle(pkl_filename)
    if adjtype == "scalap":
...
def calculate_normalized_laplacian(adj_mx):
    ...
    res = sp.eye(adj_mx.shape[0]) - adj_mx.dot(d_mat_inv_sqrt).transpose().dot(d_mat_inv_sqrt).tocoo()
    return res
...
def compute_all_metrics(preds, labels):
    ...
    mae = masked_mae(preds, labels, 0.0).item()
    mape = masked_mape(preds, labels, 0.0).item()
    rmse = masked_rmse(preds, labels, 0.0).item()
    return round(mae,6), round(mape,6), round(rmse,6)